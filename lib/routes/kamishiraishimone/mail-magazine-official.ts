...
import { KamishiraishimoneBrowser, sleep, urls } from './utils';
// const syncedMap = require('@/utils/syncedMap');
// const get_config = require('@/utils/get_config');

export const route: Route = {
    path: '/mail_magazine/official',
    name: 'mail_magazine_official',
    maintainers: ['barryZZJ'],
    handler,
    example: '/mail_magazine/official',
    features: {
        requireConfig: [
            {
...
async function handler(ctx) {
    ...
    const body = await browser.fetchOfficialMailBody();
    // logger.info('fetched body:\n'+body);
    const $ = cheerio.load(body);

    const rssTitle = 'オフィシャルメールマガジン | 上白石萌音 オフィシャルホームページ';
    ...
    let items = await Promise.all(
        items_.slice(0, detail_max).map((item) =>
